...
var vy0Ball = 0;
var dMin,
  iMin = 0; // minimale Distanz Ball - Playground, Segmentnummer

var vWind,
  vWindMax = 15 / 3.6; // Windgeschwindigkeit ~4 km/h
...
function draw() {
  ...
  if (debug) {
    ...
    text(
      "vxBall: " + nf(vxBall, 2, 3) + "    vyBall: " + nf(vyBall, 2, 3),
      14 * gridX,
      26 * gridY
    );
  ...
  if (START) {
    ...
    if (freePutter == true && putter[0] == false) {
      // Start der Putterbewegung feststellen
      INIT = true;
      status = "start";
    }
    ...
    if (next || !debug) {
      ...
      switch (
        ...
        case "2.plane":
          if (len_i[3] > s) {
            // Ortsberechnung
            console.log("s is ", s);
            console.log("xBall ", xBall);
            console.log("y Ball is ", yBall);
            g_ = g_i[3];
            beta = beta_i[3];
            len = len_i[3];
            aRR = aRR_i[3]; // wirksame Reibungsverzögerung
        ...
        case "3.plane":
          if (len_i[5] > s) {
            ...
            beta = beta_i[5];
            len = len_i[5];
            aRR = aRR_i[5]; // wirksame Reibungsverzögerung
            xBall0 = P[5][0];
            //console.log("2.plane");
          } else {
            // Ende im Wasser
            status = "hole"; // Status ändern
            s = 0; // Weg rücksetzen
            xBall0 = P[5][0];
            dt = 0;
          }
        ...
        case "4.plane":
          if (len_i[7] > s) {
            ...
            beta = beta_i[7];
            len = len_i[7];
            aRR = aRR_i[7]; // wirksame Reibungsverzögerung
            xBall0 = P[7][0];
            //console.log("2.plane");
          } else {
            // Ende im Wasser
        ...
        case "on flight":
          ...
          for (var i = 0; i < P.length - 1; i++) {
            ...
            if (result[1] <= dMin && result[2] < len_i[i] && result[2] > 0) {
              // Distanz Ball - Playground Punkt
      ...
      switch (
        ...
        case "3.slope":
          ...
          if (abs(vs) < 0.03 && beta == PI)
            // Ende der Bewegung -> Energie aufgebraucht
            dt = 0;
          else {
            if (vs > 0) sign = 1;
  ...
  vertex(
    (P[7][0] + normVWind * vWind) * M,
    (P[7][1] + flagRodHeight - 0.5 * flagLapHeight) * M
  );
