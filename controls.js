...
function PushButton(xPos, yPos, onName, onColor, modus) {
  ...
  this.drawButton = function (
    ...
    if (
      x < mouseX &&
      x + buttonWidth > mouseX &&
      y < mouseY &&
      y + buttonHeight > mouseY
    ) {
...
function Circle(r, M, visible, c, mode) {
  ...
  this.inCircle = function (
    xPos,
    yPos // Pos in Metern und im kart. Koord.Syst.
  ) {
    var R = this.r;
    var Scale = this.M;
    var insideBubble;
    ...
    /* Maus in der Ellipse? */
    if (
      sq(iXk(mouseX) - xPos * Scale) + sq(iYk(mouseY) - yPos * Scale) <=
      sq(R * Scale)
    ) {
...
function manageColor(c) {
  ...
  if (brightness > 150) return [invertedColor, "#000000", Red, Green, Blue];
  else return [invertedColor, "#ffffff", Red, Green, Blue];
}
